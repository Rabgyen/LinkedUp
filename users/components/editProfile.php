<?php


$message = "";
$pClass = "";

if ($_SERVER['REQUEST_METHOD'] == "POST") {

    $id = $_GET['id'];

    $userSql = "SELECT * FROM user_info WHERE user_id='$id'";
    $userResult = mysqli_query($conn, $userSql);
    $currentUser = mysqli_fetch_assoc($userResult);


    $full_name = !empty($_POST["full_name"]) ? $_POST["full_name"] : $currentUser['full_name'];
    $bio = !empty($_POST["bio"]) ? $_POST["bio"] : $currentUser['bio'];

    $updateNameSql = "UPDATE user_info SET full_name = '$full_name' WHERE id='{$currentUser['id']}'";
    $updateBioSql = "UPDATE user_info SET bio = '$bio' WHERE id='{$currentUser['id']}'";

    $updateName = mysqli_query($conn, $updateNameSql);
    $updateBio = mysqli_query($conn, $updateBioSql);

    if ($updateName && $updateBio) {
        $_SESSION['full_name'] = $full_name;
        $_SESSION['bio'] = $bio;
        $pClass = 'success-text';
        $message = "<i class='fa-solid fa-circle-check'></i> Changes Saved";
    }else{
        $pClass = 'fail-text';
        $message = "<i class='fa-solid fa-circle-check'></i> Error Occured";
    }



}

?>


<div class="editProfile">
    <h2>Edit Profile</h2>
    <div class="edit-profile-container">
        <div class="edit-profile-img">
            <img src="../../images/user.jpg" alt="">
            <span>
                <button type="button" id="upload-img-btn"><i class="fa-solid fa-plus"></i> Change Image</button>
                <button type="button" id="remove-img-btn">Remove Image</button>
                <input type="file" id="img-input" accept="image/*" hidden>
                <p>Change your picture anytime you like!</p>
            </span>
        </div>
        <form action="" method="post">
            <div class="edit-profile-name">
                <label for="full_name">Full Name: </label>
                <div class="input-field">
                    <input type="text" name="full_name" id="full_name" placeholder="<?php echo $_SESSION['full_name'] ?>">
                    <i class="fa-regular fa-user"></i>
                </div>
            </div>
            <div class="edit-profile-bio">
                <label for="bio">Bio: </label>
                <textarea maxlength="150" name="bio" id="bio" placeholder="Write your thoughs...."></textarea>
                <p><span id="count">0</span>/200 characters</p>
            </div>
            <button type="submit" id="saveChange">Save Changes</button><br>
            <p class="<?= $pClass ?>"><?= $message ?></p>
        </form>
    </div>
</div>
<script>
    const bio = document.getElementById("bio");
    const count = document.getElementById("count");

    bio.addEventListener("input", function() {
        count.textContent = bio.value.length;
    });

    const uploadImgBtn = document.getElementById('upload-img-btn');
    const imgInput = document.getElementById('img-input');

    uploadImgBtn.addEventListener('click', () => {
        imgInput.click();
    })
</script>